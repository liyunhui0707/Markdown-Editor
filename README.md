...
| `hybrid-cm6-lp` | Experimental (Stages A + B, opt-in) | Live-preview variant of `hybrid-cm6`. Reuses the hybrid walker via its public export PLUS adds an inline-marker plugin that emits `Decoration.replace` off the active line and registers them with `EditorView.atomicRanges` so arrow-key cursor motion steps over each hidden marker as one unit. **Stage A covered emphasis markers (`*`, `_`, `**`, `__`)**. **Stage B extends coverage to four additional inline marker categories**: inline-code backticks, strikethrough `~~`, inline-link brackets/parens/URL/title (link text stays visible underlined; Cmd-click via Stage 25 still works), and inline-image markup (alt text stays styled). On-active-line behavior matches `hybrid-cm6` exactly. Opt in via `?writeEngine=hybrid-cm6-lp` or `localStorage.setItem('markdownVault.writeEngine', 'hybrid-cm6-lp')`. First and second stages of the option-2 Obsidian-style Live Preview migration; stages C–H will extend further (image widgets, block markers, table grids, math, code highlighting, default promotion). |
...
### Live styling in `hybrid-cm6` (default) and `hybrid-cm6-lp` (Stages A + B opt-in)
...
- **Obsidian-style Live Preview migration** — Stages A and B done (`hybrid-cm6-lp` opt-in engine with `Decoration.replace`-based hiding + atomic-range cursor motion for emphasis, inline code, strikethrough, inline link markers, and inline image markup). Stages C–H upcoming: inline image widgets (C — actual `<img>` rendering), block-level marker hiding (D), tables as widgets (E), KaTeX math (F), code-block syntax highlighting + Mermaid (G), promotion to default (H). See `docs/stage-history.md` Stage A + Stage B rows for full implementation summaries.
