...
- **`hybrid-cm6-lp` (Stage A + B live-preview)** — extends the `Decoration.mark` invariant with `Decoration.replace` AND `WidgetType` widgets for inline markers. Stage A covered emphasis (`*`, `_`, `**`, `__`); **Stage B extends to four additional inline marker categories**: inline-code backticks (CodeMark with parent InlineCode), strikethrough delimiters (`~~`), inline-link brackets/parens/URL/title, and inline-image markup (alt text stays visible). Off-active-line, the lp-inline plugin emits `Decoration.replace` with an empty widget AND registers replaced ranges with `EditorView.atomicRanges` so arrow-key cursor motion steps over each hidden marker as one unit. On-active-line, lp-inline emits nothing and the hybrid walker's existing `Decoration.mark` + existing CSS reveals the marker dimmed. The raw-Markdown-as-source-of-truth invariant is preserved: `Decoration.replace` does not mutate the document. Stages C–H may extend the pattern further (image widgets, block markers, table grids, math, code highlighting, default promotion).
