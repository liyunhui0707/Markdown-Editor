...
Five Write engines coexist. Resolution priority (`lib/write-engine.js`): `?writeEngine=<name>` URL param → `localStorage.markdownVault.writeEngine` → default `hybrid-cm6`. Valid names: `hybrid-cm6` (default), `cm6` (fallback), `hybrid` (legacy fallback), `hybrid-cm6-lp` (CM6 live-preview, opt-in), `tiptap` (ProseMirror/Tiptap WYSIWYG, opt-in).

**WYSIWYG direction (post-G.13 pivot, 2026-06-07).** The CM6 decoration-based live-preview (`hybrid-cm6-lp`, Stages A–G.13) hit an inherent ceiling: toggling a tall rendered block (esp. Mermaid) to its source reflows the document, and render-only blocks have no source-position mapping — so CM6 (a line/text editor) can't make "click a rendered diagram, cursor lands where you clicked" work. The project therefore adopted vmark's model: a **`tiptap` ProseMirror WYSIWYG engine** edits a rich-text document directly (no reveal/reflow), with **Markdown still the source of truth** via a `remark` bridge. `hybrid-cm6-lp` (merged to `main` via PR #133) is retained as a base / future source-mode, NOT the WYSIWYG endgame. See memory `project_wysiwyg_prosemirror_pivot`.

- **`tiptap` (ProseMirror WYSIWYG, opt-in)** — Tiptap 3 (`@tiptap/core` + `starter-kit` + table/task-list extensions). The Markdown bridge is `lib/markdown-mdast-pm.js` (a PURE, headlessly-testable mdast↔ProseMirror-JSON converter) wired to `unified`/`remark` (`remark-parse` + `remark-gfm` + `remark-stringify`) in `lib/tiptap-entry.js` → `npm run build:tiptap` → `lib/tiptap-bundle.js`. Replaces `tiptap-markdown` (a Tiptap-2 lib that silently dropped GFM task-list checkboxes on Tiptap 3). Round-trip is **remark-NORMALIZED, not byte-identical** (the one engine where the byte-identical invariant below is relaxed — markdown is still the canonical store). The converter is TOTAL (never throws; raw HTML, reference-style links/images, and footnotes are preserved VERBATIM as `rawInline`/`rawBlock` passthrough atoms — `lib/tiptap-raw.js` — re-emitted as mdast `html` on save so they survive load→save instead of being escaped/dropped; other unknown nodes degrade to visible text) and `setText` guards `setContent` with `errorOnInvalidContent:true` + a plain-text fallback so the editor never goes blank. GFM tables render as grids; `- [ ]/[x]` as checkboxes; inline `$x$` + display `$$x$$` render via KaTeX (`lib/tiptap-math.js`, `remark-math`); ` ```mermaid ` blocks render as SVG (`lib/tiptap-mermaid.js`, async via the vendored `window.mermaid`). Math/mermaid are leaf atom nodes carrying verbatim source so they round-trip without remark escaping (which otherwise corrupts LaTeX `_`/`*`). Leading YAML frontmatter is split off and preserved byte-for-byte (`lib/markdown-frontmatter.js`), kept out of the WYSIWYG body. Images are gated through an allowlist + vault-relative IPC (`lib/tiptap-image.js` / `image-url-safety.js`; the Toast UI Preview is gated too via `lib/toast-image-renderer.js`). A **Source↔WYSIWYG toggle** (`lib/tiptap-source-toggle.js`, button in `#editorModeToggle`) swaps the Write pane to an editable raw-markdown textarea (FULL markdown incl. frontmatter) and back, sharing `note.body`; `exitWriteMode` commits source→rich so save/switch are current. Deferred: per-column table alignment; click-to-edit math/mermaid atoms in-place; CM6 syntax highlighting + cursor preservation in the source view.
