/* Phase A.2 — rendering helpers for the Codex importer.
   Ported verbatim from Local-Web-Server/tools/import-codex.js (commit 4bb2eb3)
   lines 311–554. ESM exports → CommonJS.

   Allowlist semantics preserved exactly: only `response_item` records with
   subtype `message` (role user/assistant), `function_call`, or
   `function_call_output` render. All other record types (session_meta,
   turn_context, response_item:reasoning, event_msg:*, custom_tool_call*,
   web_search*, tool_search_*, etc.) are silently dropped. computeErroredCallIds
   is a whole-file prepass over THREE signals correlated by call_id:
   - event_msg:exec_command_end.exit_code !== 0
   - event_msg:patch_apply_end.success === false
   - response_item:function_call_output.output.error (non-empty)
   Order-independent: the error signal may appear AFTER the corresponding
   function_call_output in the JSONL stream. */
...
function renderTranscript(content, options) {
...
function computeErroredCallIds(content) {
...
function extractEnrichment(content) {
  ...
  for (const line of lines) {
    ...
    if (obj.type === 'session_meta') {
      ...
      if (typeof p.cli_version === 'string' && version === null) version = p.cli_version;
