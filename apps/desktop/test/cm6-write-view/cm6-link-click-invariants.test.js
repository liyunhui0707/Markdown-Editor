/* Stage 25 — peer source-file contract for cm6-link-click.js.
   Run focused: node --test test/cm6-write-view/cm6-link-click-invariants.test.js

   The link-click module is the narrow event-surface exception for Q2
   (Cmd-click external link open). It is allowed exactly one
   EditorView.domEventHandlers call (the mousedown handler) and exactly
   one cm6.keymap.of call (the Mod-Shift-o binding), and nothing else
   from the forbidden token set. Production source comments must phrase
   guidance WITHOUT using the literal forbidden substrings because the
   scanner reads raw source. */
...
const LINK_CLICK_PATH = path.join(__dirname, '..', '..', 'lib', 'cm6-link-click.js');

test('Stage 25-10: cm6-link-click.js source-file invariants', () => {
  const src = fs.readFileSync(LINK_CLICK_PATH, 'utf8');
  ...
  assert.equal(eventHandlers.length, 1,
    'cm6-link-click.js must call EditorView.domEventHandlers exactly once');
  const keymapCalls = src.match(/cm6\.keymap\.of\s*\(/g) || [];
  assert.equal(keymapCalls.length, 1,
    'cm6-link-click.js must call cm6.keymap.of exactly once');

  // Forbidden tokens — same set as Section H plus the Stage 16-10
  // adapter-output set. Production comments must phrase guidance WITHOUT
  // using these literal substrings.
